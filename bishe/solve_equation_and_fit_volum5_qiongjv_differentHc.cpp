...
const double uppper_limit[7]={100.0e-9,100.0e-9,0.3/miu0,0.3/miu0,1.0e-7,10.0e-9,100.0e-9};
const double lower_limit[7]={1.0e-9,10.0e-9,0.15/miu0,0.3/miu0,10.0e-9,1.0e-10,10.0e-9};
...
int main()
{
  ...
  double best_position[7]={10e-7,23.0e-9,0.2/miu0,0.2/miu0,40.0e-9,3.0e-9,20.0e-9};
  ...
  #pragma omp parallel for
    ...
    for(int i1=0;i1<1000000;i1++)
    {
      ...
      Hc_1=lower_limit[2]+(uppper_limit[2]-lower_limit[2])*rand()*1.0/RAND_MAX;
      Hc_1=lower_limit[3]+(uppper_limit[3]-lower_limit[3])*rand()*1.0/RAND_MAX;
      xi_0=lower_limit[4]+(uppper_limit[4]-lower_limit[4])*rand()*1.0/RAND_MAX;
      l_1=lower_limit[5]+(uppper_limit[5]-lower_limit[5])*rand()*1.0/RAND_MAX;
      //l_2=lower_limit[6]+(uppper_limit[6]-lower_limit[6])*rand()*1.0/RAND_MAX;
      l_2=xi_0/2;
      ...
      if(fangcha<best_fit)
      {
        ...
        best_position[2]=Hc_1;
        best_position[3]=Hc_2;
        best_position[4]=xi_0;
        best_position[5]=l_1;
        best_position[6]=l_2;
      }
  ...
  Hc_1=best_position[2];
  Hc_2=best_position[3];
  xi_0=best_position[4];
  l_1=best_position[5];
  l_2=best_position[6];
  for(int j=0;j<length;j++)
  {
    cout<<data[j][0]*miu0*1000<<" "<<Q(data[j][0],2)<<" "<<exp(-2*thickness_a/(lambda_L_T_H_1(2,data[j][0])*(1+xi_0/l_1)))<<endl;
  }
  cout<<best_position[0]<<" "<<best_position[1]<<" "<<best_position[2]<<" "<<best_position[3]<<" "<<best_position[4]<<" "<<best_position[5]<<" "<<best_position[6]<<" "<<best_fit<<endl;
  cout<<Rs1(2,0)<<" "<<Rs2(2,0)<<" "<<exp(-2*thickness_a/(lambda_L_T_H_1(2,0)*(1+xi_0/l_1)))<<endl;
  return 0;
}