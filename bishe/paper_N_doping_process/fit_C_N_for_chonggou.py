...
import N_doping_chonggou as N_D
...
if __name__ == "__main__":
    ...
    C_N=3.48
    N_x_array=np.array([200,200])
    N_x=int(np.sum(N_x_array))
    total_t=20*60
    N_t=total_t*10*5
    ...
    u=np.zeros(N_x)
    result=N_D.solve_equ(parameters,N_x_array,u)
    '''
    total_t=0*60
    N_t=total_t*10

    parameters=[800+273,0,C_N,2.79e25,2.46e28,N_x,N_t,total_t,6.8*10**-14]
    parameters=np.array(parameters)
    
    result=N_D.solve_equ(parameters,N_x_array,result[0])
    '''
    x_1=np.linspace(0,N_D.thickness_a[0],N_x_array[0])
    x_2=np.linspace(N_D.thickness_a[0],N_D.thickness_a[0]+N_D.thickness_a[1],N_x_array[1]+1)
    X=np.concatenate([np.delete(x_1,-1),x_2],axis=0)
    '''
    a=0
    i=0
    while(i<42):
    ...
    print(a)
    '''
    np.save("data\_test_data_800_3N60min_3.5Pa_theory",result[0])
    plt.figure(1)
    plt.xlim(0,2*10**-6)
    plt.ylim(1e23,2.5e28)
    plt.semilogy(X,result[1],label="Theory total")
