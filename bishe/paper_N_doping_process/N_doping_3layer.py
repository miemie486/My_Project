...
u_max_2:氮的最高浓度        默认2.46e28 因为Nb2N的密度为8.02g/cm^3,就能算出此时的N浓度。且Nb2N的浓度范围是Nb_2N_0.84到Nb2N，N的浓度范围是2.06e28-2.46e28
...
@jit
def set_matrix_B_and_reaction_u(parameters,u,u_total):
    T=parameters[0]
    p=parameters[1]
    C_N=parameters[2]
    u_max_1=parameters[3]
    ...
    while(i<N_x):
        ...
        #if(i==0):
            #print(C_N,nitride_u[0])
        nitride_u[i]=nitride_u[i]+C_N*u[i]*delta_t
        i=i+1
...
def solve_equ(parameters,u):
    ...
    while(i<N_t):
        ...
        u=result_u
        total_u=B_and_reaction_u[1]+u   #nitride_u+u
        #print("-------",B_and_reaction_u[1][0])
