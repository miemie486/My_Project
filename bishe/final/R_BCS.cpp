...
{
  double aa;
  aa=lambda_L_T(T)*sqrt(2.0)/(sqrt(1+sqrt(1-pow(H/Hc_T(T),2))));
  return aa;
}
...
{
  double aa;
  aa=0.5*(Delta_T_H(T,H)/(kB*T))*log(2.246*kB*Tc/(hbar*omega))*
  (pow(omega,2)*pow(miu0,2)*pow(lambda_L_T_H(T,H),3))/(2*rou_ph*alpha(T,H));
  return aa;
}
...
{
  double aa;
  aa=C_dirty(T,0)*l_1*pow((1+xi_0/l_1),1.5)*exp(-Delta_T_H(T,H)/(kB*T));
  return 2*aa+R_res;
}
...
{
  double aa;
  aa=C_dirty(T,0)*l_2*pow((1+xi_0/l_2),1.5)*exp(-Delta_T_H(T,H)/(kB*T));
  return aa+R_res;
}
