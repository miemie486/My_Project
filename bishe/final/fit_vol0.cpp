...
const double uppper_limit[6]={33.0e-9,1.0e-7,0.2/miu0,41.0e-9,10.0e-9,100.0e-9};
const double lower_limit[6]={32.0e-9,10.0e-9,0.15/miu0,40.0e-9,1.0e-9,10.0e-9};
...
int main()
{
  ...
  double aaa[6]={160e-9,32.0e-9,0.2/miu0,40.0e-9,0.9e-9,20.0e-9};
  fitparameter.set_parameters(aaa);
  
  for(int j=0;j<4;j++)
  {
    fitparameter.set_H((0.00211236+j*0.004)/miu0);
    fitparameter.caculate_T_distribution();
    cout<<(0.00211236+j*0.004)/miu0*miu0*1000<<" "<<fitparameter.Q()<<" "<<fitparameter.T[0]<<" ";
    cout<<exp(-2*aaa[0]/(fitparameter.lambda_L_T_H(fitparameter.T[0],data[j][0])*pow(1+aaa[3]/aaa[5],0.5)))<<endl;
  }
  ...
  for(int j=0;j<length;j++)
  {
    ...
    fitparameter.caculate_T_distribution();
    cout<<data[j][0]*miu0*1000<<" "<<fitparameter.Q()<<endl;//" "<<fitparameter.T[0]<<" ";
    //cout<<exp(-2*aaa[0]/(fitparameter.lambda_L_T_H(fitparameter.T[0],data[j][0])*pow(1+aaa[3]/aaa[5],0.5)))<<endl;
  }
  cout<<fitparameter.thickness_a<<" "<<fitparameter.lambda_L<<" "<<fitparameter.Hc<<" "<<
  fitparameter.xi_0<<" "<<fitparameter.l_1<<" "<<fitparameter.l_2<<endl;
	return 0;
}