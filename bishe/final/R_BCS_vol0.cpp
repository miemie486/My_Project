...
{
  double aa;
  aa=C_dirty(T,0)*l_1*pow((1+xi_0/l_1),1.5)*exp(-Delta(T)/(kB*T));
  return aa+R_res*(1+5.5*pow(H/(0.6*Hc_T(T)),6));
}

double two_layer_model::Rs2(double T, double H) 
{
  double aa;
  aa=C_dirty(T,0)*l_2*pow((1+xi_0/l_2),1.5)*exp(-Delta(T)/(kB*T));
  return aa+R_res;
}
