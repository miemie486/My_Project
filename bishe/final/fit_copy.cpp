...
const double Maxv[dimension]={1.0e-7,1.0e-9,10.0e-9,0.5,0.5};
const double uppper_limit[dimension]={150.0e-9,10.0e-9,50.0e-9,0.5,0.5};
const double lower_limit[dimension]={1.0e-9,1.0e-9,10.0e-9,-0.5,-0.5};
/*
...
int main()
{
  ...
  //初始化速度与位置
    ...
    cout<<i<<" "<<_position_best_globle_fit<<endl;
  ...
  /*
	//mycout.close();
  double aaa[5]={3.59474e-08,6.83416e-09,3.2033e-08,0.407476,-0.476688};
  fitparameter.set_parameters(aaa);
  cout<<endl<<fitparameter.fit(length,data)<<endl;
  */
  for(int j=0;j<length;j++)
  {
    fitparameter.set_H(data[j][0]);
    fitparameter.caculate_T_distribution();
    cout<<data[j][0]*miu0*1000<<" "<<fitparameter.Q()<<endl;//" "<<fitparameter.Delta_T_H(fitparameter.T[0],data[j][0])/(9.23*1.38e-23)<<endl;
  }
  ...
  double kappa1=fitparameter.lambda_L_T_H(fitparameter.T[0],0)*pow((1+fitparameter.xi_0/fitparameter.l_1),0.5)/(1/(1/fitparameter.xi_0+1/fitparameter.l_1));
  double kappa2=fitparameter.lambda_L_T_H(fitparameter.T[0],0)*pow((1+fitparameter.xi_0/fitparameter.l_2),0.5)/(1/(1/fitparameter.xi_0+1/fitparameter.l_2));
  cout<<kappa1<<" "<<kappa2<<" "<<
