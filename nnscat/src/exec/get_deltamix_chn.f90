...
program get_deltamix_chn
  ...
  if  (ios /= 0) then
    write (standard_error_unit, '(a)')  &
      & 'fit_deltamix_chn: Something wrong with the arguments'
    write (standard_error_unit, '(a)')  &
      & 'Usage: fit_deltamix_chn N chn uptoQn opt [setn_cis]'
    stop
  end if
  select case (trim(opt))
    ...
    case default
          write (standard_error_unit, '(a)')  &
            & 'fit_deltamix_chn: unknown option '//trim(opt)
  end select
  if (uptoQn == 3)  then
    read (cmdbuff, *, IOSTAT=ios) N, chnstr, uptoQn, opt, setn_cis
    if  (ios /= 0) then
      write (standard_error_unit, '(a)')  &
        & 'fit_deltamix_chn: for uptoQn = 3, setn_cis is needed'
      write (standard_error_unit, '(a)')  &
        & 'Usage: fit_deltamix_chn N chn uptoQn opt [setn_cis]'
      stop
    end if
    ! if (setn_cis < 0 .or. setn_cis > 4) then
    !   write (standard_error_unit, '(a)') 'fit_deltamix_chn: setn_cis = 1 - 4'
    !   stop
    ! else
