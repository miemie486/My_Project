! pray  OCT/7/2019
! program caculate phase shift with SFR poential deltaless only
! for UPtoQn = 3 P-wave only test counter terms

program get_supsfr_smpl_contacttest
...
print *, 'setn_cis', setn_cis
  ...
  if (.not. successful) then
    write  (standard_error_unit, '(a)')  &
      & 'get_supsfr_smpl_contacttest: converting '//cmdstr//' wasn''t successful'
    stop
  end if

  allocate(obj_smplchn::chn)
  call create_smplchn(chn, chnid, uptoQn)
  call SetUp_pope_SFR_smpl_test(chn)
...
end program get_supsfr_smpl_contacttest