...
program fit_lbwdib
  ...
  if  (ios /= 0) then
    write (standard_error_unit, '(a)')  &
      & 'fit_lbwdib: Something wrong with the arguments'
    write (standard_error_unit, '(a)')  &
      & 'Usage: fit_lbwdib Nmesh uptoQn [setn_cis]'
    stop
  end if
  if (uptoQn >= 3) then
    ...
    if  (ios /= 0) then
      write (standard_error_unit, '(a)')  &
        & 'fit_lbwdib: for uptoQn = 3, setn_cis is needed'
      write (standard_error_unit, '(a)')  &
        & 'Usage: fit_lbwdib Nmesh uptoQn [setn_cis]'
      stop
    end if
    if (setn_cis < 0 .or. setn_cis > 4) then
      write (standard_error_unit, '(a)') 'fit_lbwdib: setn_cis = 1 - 4'
      stop
    else
  ...
  if (uptoQn == 0) then
    ...
    npar_known = npar_lbwdib(uptoQn)
  end if
  npar_unknown = npar_lbwdib(uptoQn+1) - npar_known
  call read_inpar_fitting(inpar_fname, npar_known+npar_unknown, Mambda_inpar,  &
    & nMmbd)

  do ii = 1, nMmbd
    ! call Fit1s0lbwdib_fitting(Nmesh, Mambda_inpar(ii, 1), uptoQn,              &
    !   & Mambda_inpar(ii, 2:npar_known+1),                                      &
    !   & kcm_fitphs(:, 1), numk, kcm_fitphs(:, 2),                              &
    !   & Mambda_inpar(ii, npar_known+2:npar_known+1+npar_unknown), minchisqr)
    call FitSngl_fitting(Nmesh, Mambda_inpar(ii, 1), chnid, uptoQn,     &
      ...
      & minchisqr, Get1s0dib_fitpwa, 1000)
  end do
