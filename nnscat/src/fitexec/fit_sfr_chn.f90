...
select case(trim(cmdstr))
  ...
  errmsg = 'fit_sfr_chn: Something went wrong'

  call PrintOutFitcpld_fitting(GetPS_cpld_popedeltaless,                         &
  ...
  call PrintOutFitSngl_fitting(GetPs_sngl_popedeltaless,                         &
...
contains
  subroutine GetPS_cpld_popedeltaless(Nmesh, Mambda, chnid, uptoQn, paralst,      &
  ...
  end subroutine GetPS_cpld_popedeltaless

  subroutine GetPS_sngl_popedeltaless(Nmesh, Mambda, chnid, uptoQn, paralst,      &
    & num_paras, klist, numk, phaselst)
    real(NER), intent(in)   :: Mambda, paralst(:), klist(:)
    integer, intent(in)     :: Nmesh, uptoQn, chnid, num_paras, numk
...
!       print *, ptrchn%inparas(1), ptrchn%phsn(2), ptrchn%phsn(3), ptrchn%phsn(4)
  ...
  end subroutine GetPS_sngl_popedeltaless


 end program fit_sfr_chn